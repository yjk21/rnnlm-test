...
void CRnnLM::testNet()
{
    ...
    while (1) {
        ...
        if ((word!=-1) || (prob_other>0)) {
        	...
        	log_combine+=log(0 * lambda + prob_other*(1-lambda));
    	    } else {
    		logp+=log(neu2[vocab[word].class_index+vocab_size].ac * neu2[word].ac);
        	log_combine+=log(neu2[vocab[word].class_index+vocab_size].ac * neu2[word].ac*lambda + prob_other*(1-lambda));
    	    }
    	    log_other+=log(prob_other);
            wordcn++;
        }
...
void CRnnLM::testNbest()
{
	...
	if (lambda>0) computeNet(last_word, word);      //compute probability distribution
        ...
        if (word!=-1) {
            logp+=log(neu2[word].ac);
    	    
            log_other+=log(prob_other);
            
            log_combine+=log(neu2[word].ac*lambda + prob_other*(1-lambda));
            
            senp+=log(neu2[word].ac*lambda + prob_other*(1-lambda));
